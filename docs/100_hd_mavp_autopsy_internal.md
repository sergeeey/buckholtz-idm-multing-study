...
| **A5** | **Внешние тесты (BBN / PPN / Bullet Cluster)** | IDM 5 изомеров + локальная гравитация | Теория внутренне согласована и готова к внешним данным | Нарушение BBN (N_eff), орбит (PPN), столкновений (Bullet) | Всё заблокировано до SC-2 + A0 resolved | Не начато: нет смысла пока bridge неизвестен | **M7-C [2026-06-12]:** N_eff BLOCKED — 6/7 thermal ingredients NOT_FOUND в preprint (T_dark, decoupling T, entropy history, g_D, coupling, ΔN_eff). Только I5 AUTHOR_HINTED (dark particles might exist). Planck 2018 bound неприменим. See `reports/m7_c_thermal_history_neff_audit.md`. | `[MCMC_BLOCKED / N_EFF_BLOCKED]` | Freeze all astro simulations. Внутренний разрыв ×4365 делает внешние тесты бессмысленными. BBN/N_eff separately blocked: thermal history absent from preprint. |
