...
(4)~an integer IDM isomer count $N=5$ is excluded at $5.67\sigma$ by Planck 2018;
...
With the Belle~II target $\sigma_{m_\tau} = 0.01$~MeV, the combined significance
would rise to $1.90\sigma$ — an approaching but not yet decisive test.
...
ranging from $+0.0014\%$ (Luo et al.~2009) to $+0.032\%$ (Quinn et al.~2014),
...
of Eq.~(\ref{eq:c9}) from equality ($135$~ppm with CODATA 2018), implying that
current $G$ precision \emph{prevents} a definitive test of whether
Eq.~(\ref{eq:c9}) is exact.  The value of $G$ that makes Eq.~(\ref{eq:c9})
exact is $G^* = 6.6734 \times 10^{-11}$~m$^3$\,kg$^{-1}$\,s$^{-2}$
(within the CODATA 2018 $1\sigma$ band); the Luo et al.~2009 measurement
...
The Planck 2018 bound is $\Delta N_\mathrm{eff} < 0.29$ (95\% CL, from
$N_\mathrm{eff} = 2.99 \pm 0.17$); the minimal IDM scenario exceeds this by
$>100\sigma$.
\reconstruction
...
(quantified in §\ref{sec:discuss}: SM entropy dilution to
$g_{*S} = 106.75$ gives $\Delta N_\mathrm{eff} = 0.70$ for 15 dark-neutrino
species, still $2.4\times$ over the limit),
...
denotes the octonions; independently, $12 = |W(\mathrm{G}_2)|$, the order of the
Weyl group of $\mathrm{G}_2$ --- two algebraic derivations of the same integer;
...
structure (Baez--Huerta, Todorov--Dubois-Violette); the specific connection
...
$m_\tau$ to $\pm0.01$~MeV & Belle~II & 2027 \\
...
IDM $N=5$ integer & $\times$ $5.67\sigma$ & Planck 2018 \\
...
First, $N=5$ isomers are excluded at $5.67\sigma$ by Planck 2018.
Second, dark-sector neutrinos (15 species for five MESI isomers $\times$ three
generations) contribute $\Delta N_\mathrm{eff} = 0.70$ under the most
favourable SM thermal history ($g_{*S} = 106.75$), rising to
$\Delta N_\mathrm{eff} \approx 22$--81 for the full mirror sector without
entropy advantage; both exceed the Planck 2018 limit $\Delta N_\mathrm{eff} <
0.29$ (95\% CL).
A massive-neutrino escape route fails because it implies
