...
ratio $\alpha_\mathrm{EM}/\alpha_G$. A targeted literature search did not
locate this specific relation in the indexed literature.
...
($84$ rational prefactors $p/q$ with $p,q\le 6$, integer exponents
...
holds at $0.00092\%$ (PDG 2024) and is symmetric under all six
permutations of $\{m_e, m_\mu, m_\tau\}$ (i.e., $S_3$-invariant).
Substituting the Koide-exact value of $m_\tau$ into Eq.~(32) predicts
$\alpha_G = 1.7503 \times 10^{-45}$, agreeing with the standard value
(from CODATA $G$) to $0.087\%$.
If both relations are simultaneously exact, Newton's constant $G$ is
determined from $\alpha_\mathrm{EM}$, $m_e$, and $m_\mu$ alone,
without $m_\tau$ as a free parameter.
This is a non-trivial cross-constraint; its mechanism is unknown.
\reconstruction
