...
\textbf{T-IDM (two-component necessity, by exhaustion).}
Across 8 independent functional families tested (virial $k_A(z)$, merger-rate,
log-Gaussian, Lognormal, $D^n H^m$ power products, and merger-epoch $r_P$),
no single-component formula achieves Pearson $r > 0.85$ on all 11 $\varepsilon(z)$
points; the empirical ceiling is $r = 0.820$ for the unconstrained $D^{5.5} H^{2.75}$
power product.  This establishes by exhaustion that the secondary bump at $z = 8.50$
requires a physically distinct second term, consistent with MULTING's decomposition
$F_\mathrm{oP} = F_m - F_d + F_q$, where the quadrupole $F_q$ (parameter $\beta_q$)
dominates at high redshift (null-result series NR-001 through NR-008, see supplementary).
\reconstruction
